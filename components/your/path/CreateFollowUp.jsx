...
const CreateFollowUp = ({ closeDialog }) => {
  ...
  const mutation = useMutation({
    mutationFn: async (data) => await apiReq.post('api/follow-up/create', data, { headers: { Authorization: token } }),
    onSuccess: (res) => {
  ...
  return (
    <Stack gap={2}>
      ...
      <CButton loading={mutation.isPending || fileUploadLoading} variant="contained" color="primary" onClick={handleSubmit}>Submit</CButton>
    </Stack>
  )
}
